...
def train_emotion_model(model_name):
    with open('pre-processed-data/sentiment/avg_train_X.data', 'rb') as w:
        train_X = pickle.load(w)
    with open('pre-processed-data/sentiment/avg_dev_X.data', 'rb') as w:
        dev_X = pickle.load(w)
    ...
    train_y = [np.argmax(x, axis=0) / 5 + 0.0001 for x in train_y]
    dev_y = [np.argmax(x, axis=0) / 5 + 0.0001 for x in dev_y]

    def multimodal_emotion_model(text_shape, acoustic_shape, visual_shape):
        text_input = Input(shape=text_shape, name='text_input')
        text_lstm = LSTM(512, return_sequences=True)(text_input)

        acoustic_input = Input(shape=acoustic_shape, name='acoustic_input')
        acoustic_lstm = LSTM(512, return_sequences=True)(acoustic_input)

        visual_input = Input(shape=visual_shape, name='visual_input')
        visual_lstm = LSTM(512, return_sequences=True)(visual_input)
        ...
        dense1 = Dense(512, activation='relu')(multimodal_features)
    ...
    # Train the model
    def train_model(train_X, train_y, dev_X, dev_y, batch_size=128, epochs=5):
        text_shape = (train_X[0][0].shape[0], train_X[0][0].shape[1],)
        acoustic_shape = (train_X[0][0].shape[0], train_X[1][0].shape[1],)
        visual_shape = (train_X[0][0].shape[0], train_X[2][0].shape[1],)
        ...
        for epoch in range(epochs):
            ...
            logs.update({'val_' + name: v for name, v in validation_logs.items()})
            callbacks.on_epoch_end(epoch, logs)
            model.save('trained_model/' + model_name + str(epoch) + '.h5')
...
def evaluation(model_path, test_X, test_y, output_stream):
    ...
    output_stream.write("Sentiment Multimodal Model F1: " + str(get_f1(test_y, pred_y)) + '\n')
    output_stream.write("Sentiment Multimodal Model MAE: " + str(get_mae(test_y, pred_y)) + '\n')
    output_stream.write("Sentiment Multimodal Model accuracy (binary): " + str(get_accuracy_2(test_y, pred_y)) + '\n')
    output_stream.write(
        "Sentiment Multimodal Model accuracy (7 class classification): " + str(get_accuracy_7(test_y, pred_y)) + '\n')
    output_stream.write("Sentiment Multimodal Model R^2: " + str(coeff_determination(test_y, pred_y)) + '\n')
...
def evaluation_emotion(model_name, model_path, test_X, test_y, output_stream):
    text_dev = np.array([x[0] for x in test_X])
    acoustic_dev = np.array([x[1] for x in test_X])
    visual_dev = np.array([x[2] for x in test_X])
    model = load_model(model_path)
    ...
    baseline_acc, acc = get_emotion_acc(pred_y, test_y)
    output_stream.write('Model ' + model_name + ' accuracy: ' + str(acc) + '\n')


def evaluate_SVR(dev, output_stream):
    if dev:
        with open('pre-processed-data/sentiment/avg_dev_X.data', 'rb') as w:
    ...
    with open('trained_model/SVR.pkl', 'rb') as f:
        ...
        pred_y = np.asarray(pred_y)
        test_y = np.asarray(test_y)
        output_stream.write("Sentiment SVR Model F1: " + str(get_f1(test_y, pred_y)) + '\n')
        output_stream.write("Sentiment SVR Model MAE: " + str(get_mae(test_y, pred_y)) + '\n')
        output_stream.write("Sentiment SVR Model accuracy (binary): " + str(get_accuracy_2(test_y, pred_y)) + '\n')
        output_stream.write(
            "Sentiment SVR Model accuracy (7 class classification): " + str(get_accuracy_7(test_y, pred_y)) + '\n')
        output_stream.write("Sentiment SVR Model R^2: " + str(coeff_determination(test_y, pred_y)) + '\n')


def get_emotion_acc(pred_y, real_y):
    """
        :return baseline acc (predict everything as 0), model acc
    """
    pred_y = np.floor(np.asarray(pred_y) * 5)
    test_y = np.asarray(real_y)
    tp = 0
    zero_cnt = 0
    for i in range(len(test_y)):
        ...
        if 0 in real_indices:
            zero_cnt += 1
    return zero_cnt / len(pred_y), tp / len(pred_y)


def evaluate_SVR_emotion(dev, output_stream):
    if dev:
        with open('pre-processed-data/sentiment/avg_dev_X.data', 'rb') as w:
            test_X = pickle.load(w)
        with open('pre-processed-data/emotion/dev_y.data', 'rb') as w:
    ...
    with open('trained_model/SVR_emotion.pkl', 'rb') as f:
        ...
        pred_y = model.predict(text)
        baseline_acc, acc = get_emotion_acc(pred_y, test_y)
        output_stream.write('Emotion Model Baseline: ' + str(baseline_acc) + '\n')
        output_stream.write('Emotion SVR Model ACC: ' + str(acc) + '\n')


def evaluate_multimodal_sentiment(dev, output_stream):
    if dev:
        with open('pre-processed-data/sentiment/avg_dev_X.data', 'rb') as w:
            test_X = pickle.load(w)
        with open('pre-processed-data/sentiment/dev_y.data', 'rb') as w:
        ...
        with open('pre-processed-data/sentiment/test_y.data', 'rb') as w:
            test_y = pickle.load(w)
    evaluation('trained_model/sentiment_best_model.h5', test_X, test_y, output_stream)


def evaluate_emotion_model(model_name, dev, output_stream):
    if dev:
        with open('pre-processed-data/sentiment/avg_dev_X.data', 'rb') as w:
            test_X = pickle.load(w)
        with open('pre-processed-data/emotion/dev_y.data', 'rb') as w:
    ...
    # for i in range(0, 5):
    #     print('Epoch ' + str(i))
    #     evaluation_emotion('trained_model/' + model_name + str(i) + '.h5', test_X, test_y)
    evaluation_emotion(model_name, 'trained_model/' + model_name + '.h5', test_X, test_y, output_stream)


def generate_sentiment_output(dev):
    if dev:
        with open('pre-processed-data/sentiment/avg_dev_X.data', 'rb') as w:
            test_X = pickle.load(w)
        with open('pre-processed-data/sentiment/dev_y.data', 'rb') as w:
            test_y = pickle.load(w)
        to_write = '../../outputs/D4/primary/devtest'
    else:
        with open('pre-processed-data/sentiment/avg_test_X.data', 'rb') as w:
            test_X = pickle.load(w)
        with open('pre-processed-data/sentiment/test_y.data', 'rb') as w:
            test_y = pickle.load(w)
        to_write = '../../outputs/D4/primary/evaltest'
    text_dev = np.array([x[0] for x in test_X])
    acoustic_dev = np.array([x[1] for x in test_X])
    visual_dev = np.array([x[2] for x in test_X])
    model = load_model('trained_model/sentiment_best_model.h5')
    prediction = model.predict([text_dev, acoustic_dev, visual_dev])
    prediction = np.asarray(prediction) * 6 - 3
    with open(to_write, 'w', encoding='utf-8') as w:
        w.write('INSTANCE_ID\tSENTIMENT_GOLD\tSENTIMENT_PREDICT\n')
        for i in range(len(test_y)):
            w.write(str(i) + '\t' + str(np.round(test_y[i] * 6 - 3, 5)) + '\t' + str(
                np.round(prediction[i][0][0], 5)) + '\n')


emotion_labels = {0: 'happy', 1: 'sad', 2: 'angry', 3: 'disgust', 4: 'surprise', 5: 'fear'}


def generate_emotion_output(dev):
    if dev:
        with open('pre-processed-data/sentiment/avg_dev_X.data', 'rb') as w:
            test_X = pickle.load(w)
        with open('pre-processed-data/emotion/dev_y.data', 'rb') as w:
            test_y = pickle.load(w)
        to_write = '../../outputs/D4/adaptation/devtest'
    else:
        with open('pre-processed-data/sentiment/avg_test_X.data', 'rb') as w:
            test_X = pickle.load(w)
        with open('pre-processed-data/emotion/test_y.data', 'rb') as w:
            test_y = pickle.load(w)
        to_write = '../../outputs/D4/adaptation/evaltest'
    text_dev = np.array([x[0] for x in test_X])
    acoustic_dev = np.array([x[1] for x in test_X])
    visual_dev = np.array([x[2] for x in test_X])
    model = load_model('trained_model/emotion_multimodal_best.h5')
    prediction = model.predict([text_dev, acoustic_dev, visual_dev])
    pred_y = []
    for i in range(len(test_y)):
        pred_y.append(prediction[i][0][:])
    ...
    test_y = np.asarray(test_y)
    with open(to_write, 'w', encoding='utf-8') as w:
        w.write('INSTANCE_ID\tEMOTION_GOLD\tEMOTION_PREDICT\n')
        for i in range(len(test_y)):
            max_v = np.max(test_y[i])
            real_indices = np.where(test_y[i] == max_v)[0]
            w.write(str(i) + '\t')
            for j in range(len(real_indices) - 1):
                w.write(emotion_labels[real_indices[j]] + '|')

            w.write(emotion_labels[real_indices[len(real_indices) - 1]] + '\t' + emotion_labels[pred_y[i][0]] + '\n')
...
# train_emotion_model('emotion_visual')

print('Generating primary (sentiment) outputs...')
generate_sentiment_output(dev=True)
generate_sentiment_output(dev=False)

print('Generating adaptation (emotion) outputs...')
generate_emotion_output(dev=True)
generate_emotion_output(dev=False)

print('Please check output/D4 for the outputs.')
print()
print('Generating results...')
sentiment_dev_score_out = open('../../results/D4/primary/devtest/D4_scores.out', 'w', encoding='utf-8')
sentiment_test_score_out = open('../../results/D4/primary/evaltest/D4_scores.out', 'w', encoding='utf-8')
emotion_dev_score_out = open('../../results/D4/adaptation/devtest/D4_scores.out', 'w', encoding='utf-8')
emotion_test_score_out = open('../../results/D4/adaptation/evaltest/D4_scores.out', 'w', encoding='utf-8')

evaluate_SVR(True, sentiment_dev_score_out)
evaluate_SVR(False, sentiment_test_score_out)
sentiment_dev_score_out.write('\n')
sentiment_test_score_out.write('\n')

evaluate_multimodal_sentiment(True, sentiment_dev_score_out)
evaluate_multimodal_sentiment(False, sentiment_test_score_out)

evaluate_SVR_emotion(True, emotion_dev_score_out)
evaluate_SVR_emotion(False, emotion_test_score_out)

evaluate_emotion_model('emotion_text_best', True, emotion_dev_score_out)
evaluate_emotion_model('emotion_acoustic_best', True, emotion_dev_score_out)
evaluate_emotion_model('emotion_visual_best', True, emotion_dev_score_out)
evaluate_emotion_model('emotion_multimodal_best', True, emotion_dev_score_out)

evaluate_emotion_model('emotion_text_best', False, emotion_test_score_out)
evaluate_emotion_model('emotion_acoustic_best', False, emotion_test_score_out)
evaluate_emotion_model('emotion_visual_best', False, emotion_test_score_out)
evaluate_emotion_model('emotion_multimodal_best', False, emotion_test_score_out)

print('Please check results/D4 for the results.')
